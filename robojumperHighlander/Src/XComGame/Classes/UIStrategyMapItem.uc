...
// robojumperHighlander: with a controller, we only show pips when we focus the item.
// on PC (M+KB), we can't focus the items, so show them always
var bool bShouldOnlyShowOnFocus;
...
simulated function UIStrategyMapItem InitMapItem(out XComGameState_GeoscapeEntity Entity)
{
	...
	bShouldOnlyShowOnFocus = `ISCONTROLLERACTIVE;

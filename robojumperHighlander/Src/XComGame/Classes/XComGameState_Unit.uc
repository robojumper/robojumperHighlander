...
function EventListenerReturn OnAbilityActivated(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
	...
	if( IsConcealed() )
	{
		...
		// robojumperHighlander: Give effects a chance to madofy the concealrule
		bRetainConcealment = CheckForEffectRetainConcealment(bRetainConcealment, ActivatedAbilityStateContext);
		if (!bRetainConcealment)
			BreakConcealment();
	}
...
function bool CheckForEffectRetainConcealment(bool bRetainConcealment, XComGameStateContext_Ability ActivatedAbilityStateContext)
{
	local XComGameState_Effect EffectState, HighestRankEffect;
	local StateObjectReference EffectRef;
	local X2Effect_Persistent EffectTemplate;
	local ERetainConcealmentResult TempRule, CurrentRule;
	local XComGameStateHistory History;

	History = `XCOMHISTORY;

	
	CurrentRule = eRetainConcealment_DoesNotModify;
	...
	foreach AffectedByEffects(EffectRef)
	{
		...
		if( EffectState != none )
		{
			...
			{
				TempRule = EffectTemplate.ModifyRetainConcealmentRule(ActivatedAbilityStateContext, EffectState);

				if( (TempRule != eRetainConcealment_DoesNotModify) && (CurrentRule != TempRule) &&
	...
	switch (CurrentRule)
	{
		case eRetainConcealment_DoesNotModify:
			return bRetainConcealment;
		case eRetainConcealment_DoesBreakConcealment:
			return false;
		case eRetainConcealment_DoesRetainConcealment:
			return true;
	}
...
function XComGameState_Item GetBestUtilityItem(XComGameState NewGameState)
{
	...
	// robojumperHighlander: Fix ammo / vest utility items

...
//Runtime data
// robojumperHighlander: private->protected to allow subclassing 
var protected AnimNotify_FireWeaponVolley VolleyNotify;   //Reference to the animation notify that created us, if any
var protected X2Action_Fire FireAction;                   //Reference to the fire action we are a part of. This is the actor that created us
var protected XComGameStateContext_Ability SourceAbility; //Reference to the game state ability context being visualized by X2Action_Fire
var protected EAbilityHitResult AbilityContextHitResult;
var protected vector AbilityContextTargetLocation;
var protected int AbilityContextPrimaryTargetID;
var protected int AbilityContextAbilityRefID;
var protected bool bWasHit;                               //Tracks whether the ability that created this projectile hit the target or not. Some abilities have different looking hit / miss effects
var protected bool bIsASuppressionEffect;                 //Track if this is a suppression effect.  This flag is set and cached during the projectile's setup.
var protected XComWeapon SourceWeapon;                    //Reference to the weapon firing this projectile volley
var protected array<ProjectileSpreadValues> RandomSpreadValues; //We want each volley to have its own random trajectory, but within each volley the different projectile elements should take the same path
var protected X2WeaponTemplate WeaponTemplate;
var protected AbilityInputContext StoredInputContext;     //Keeps the input context for the ability responsible for these projectiles
var protected AbilityResultContext StoredResultContext;   //Keeps the result context for the ability responsible for these projectiles


var protected bool bCosmetic;                             //Indicates that this projectile is just for show
var protected Actor CosmeticSource;                       //If the projectile is cosmetic, this actor represents the source
var protected vector CosmeticSourceLocation;              // If the projectile is cosmetic, location represents the source. This will be ignored if the CosmeticSource is not none.
var protected Actor CosmeticTarget;                       //If the projectile is cosmetic, this location represents the target
var protected vector CosmeticTargetLocation;              // If the projectile is cosmetic, location represents the target. This will be ignored if the CosmeticTarget is not none.
var protected bool bCosmeticShouldHitTarget;              // Usually a cosmetic projectile does not have to hit the target, this ensures it does

var protected array<ProjectileElementInstance> Projectiles;//Holds a list of projectile instances and their supporting data
var protected bool bSetupVolley;                           //Lets the cleanup process know that projectile initialization has occurred and it is safe to destroy this object.
var protected bool bPlayedMetaHitEffect;                   //A latch for making sure the Meta Effect only plays once
var protected name OrdnanceType;
var bool bFirstShotInVolley;							//Indicate this is the first shot in the volley
var protected bool bProjectileFired;						//Indicate that at least a shot has been fired;

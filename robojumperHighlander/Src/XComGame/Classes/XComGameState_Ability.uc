...
simulated function UpdateAbilityAvailability(out AvailableAction Action)
{
	...
	if (kUnit != none)
	{
		...
		if (Action.AvailableCode == 'AA_Success')
		{
			...
			Action.AvailableCode = GatherAbilityTargetsScript(Action.AvailableTargets);
...
// robojumperHighlander: Allow detection of non-native targeting code, call stuff manually
simulated function name GatherAbilityTargetsScript(out array<AvailableTarget> Targets, optional XComGameState_Unit OverrideOwnerState)
{
	local int i, j;
	...
	if (m_Template != None)
	{
		// robojumperHighlander: if both are native, we don't need to call it from UnrealScript. Use fast native code
		if (class'robojumperHighlander'.default.NativeTargetStyles.Find(m_Template.AbilityTargetStyle.Class.Name) != INDEX_NONE 
			&& (m_Template.AbilityMultiTargetStyle == none 
				|| class'robojumperHighlander'.default.NativeTargetStyles.Find(m_Template.AbilityMultiTargetStyle.Class.Name) != INDEX_NONE))
		{
			return GatherAbilityTargets(Targets, OverrideOwnerState);
		}
